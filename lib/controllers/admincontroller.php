...
class AdminController extends Controller
{
    ...
    public function downloadLocationNamesAction(): array
    {
        ...
        if (empty($contentFromDb) || count($contentFromDb) === 0) {
            return [
                'message' => Loc::getMessage('kit.regions_ADMIN_ALL_CONTROLLER_ERRORS'),
            ];
        }
        ...
        foreach ($content as $value) {
            if (count($errors) > 10) {
                return [
                    'message' => Loc::getMessage('kit.regions_ADMIN_CONTROLLER_ERRORS'),
                    'data' => $errors,
                ];
            }
            if ($value[0] === '') {
            ...
            try {
                ...
                $result = ['OK' => str_replace('###', $sucssecCounter, Loc::getMessage('kit.regions_ADMIN_CONTROLLER_SUCCESS'))];
                $result[Loc::getMessage('kit.regions_ADMIN_CONTROLLER_ERRORS')] = $e->getMessage();
                return $result;
            }
        ...
        if (count($errors) !== 0) {
            $result[Loc::getMessage('kit.regions_ADMIN_CONTROLLER_ERRORS')] = $errors;
        }
    ...
    public function downloadFileAction(string $name): BFile
    {
        ...
        $path = "/bitrix/modules/kit.regions/lib/sypexgeo/{$year}_sypexGeoUpdate.log";
    ...
    {
        $controller = "kit:regions.AdminController.{$action}";
