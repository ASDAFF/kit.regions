...
use Sotbit\Regions\SypexGeo\SypexGeoUpdater;
...
class sotbit_regions extends CModule
{
    ...
    {
        EventManager::getInstance()->registerEventHandler('main', 'onMainGeoIpHandlersBuildList', 'sotbit.regions', '\\' . \Sotbit\Regions\EventHandlers::class, "addGeoIpServes",);
//        EventManager::getInstance()->registerEventHandler('main', 'onMainGeoIpHandlersBuildList', 'sotbit.regions', '\\Sotbit\Regions\EventHandlers::class, "addGeoIpServes"',);
        ...
        while ($_1398202006 = $_1516249333->fetch()) {
            $this->SotbitRegionsInstallData($_1398202006['LID']);
            $this->SotbitRegionsSetSettings($_1398202006['LID']);
        }
        return true;
    }

    function SotbitRegionsInstallData($_1260573971 = '')
    {
        $this->SotbitRegionsInstallProperties($_1260573971);
        $this->SotbitRegionsInstallDomains($_1260573971);
        $this->SotbitRegionsInstallFavorites($_1260573971);
        return true;
    }

    function SotbitRegionsInstallProperties($_1260573971)
    {
        $_1149498196 = array();
        ...
        $_1649592066 = array('ENTITY_ID' => 'SOTBIT_REGIONS', 'SORT' => 100, 'MULTIPLE' => 'N', 'MANDATORY' => 'N', 'SHOW_FILTER' => 'N', 'SHOW_IN_LIST' => 'Y', 'EDIT_IN_LIST' => 'Y', 'IS_SEARCHABLE' => 'N', 'SETTINGS' => array(), 'EDIT_FORM_LABEL' => array(), 'LIST_COLUMN_LABEL' => array(), 'LIST_FILTER_LABEL' => array(), 'ERROR_MESSAGE' => array(), 'HELP_MESSAGE' => array(),);
        ...
        foreach ($_1368976908 as $_1723785441 => $_1684220390) {
            ...
            foreach ($_1149498196 as $_434315318) {
                $_1145877735['EDIT_FORM_LABEL'][$_434315318] = Loc::getMessage('sotbit.regions_PROP_' . $_1723785441);
                $_1145877735['LIST_COLUMN_LABEL'][$_434315318] = Loc::getMessage('sotbit.regions_PROP_' . $_1723785441);
                $_1145877735['LIST_FILTER_LABEL'][$_434315318] = Loc::getMessage('sotbit.regions_PROP_' . $_1723785441);
                $_1145877735['ERROR_MESSAGE'][$_434315318] = Loc::getMessage('sotbit.regions_PROP_' . $_1723785441);
                $_1145877735['HELP_MESSAGE'][$_434315318] = Loc::getMessage('sotbit.regions_PROP_' . $_1723785441);
            }
            $_904595014 = $_135703835->Add($_1145877735);
        }
    }

    function SotbitRegionsInstallDomains($_1260573971)
    {
        $_79589753 = array();
        $_2110945885 = array();
        ...
        foreach ($_957705035 as $_52041450 => $_69734081) {
            ...
            $_1944572090 = array('CODE' => $_1852526104, 'NAME' => Loc::getMessage('sotbit.regions_DOMEN_' . $_69734081), 'SORT' => 100, 'PRICE_CODE' => $_79589753, 'STORE' => $_2110945885, 'SITE_ID' => [$_1260573971]);
            if ($_52041450 == 0) $_1887043063 = '\'Y\''; else $_1887043063 = 'NULL';


            $DB->Query("INSERT INTO `sotbit_regions` VALUES (NULL,'" . $_1944572090["CODE"] . "', '" . $_1944572090["NAME"] . "', 100, '" . serialize($_1944572090["SITE_ID"]) . "', '" . serialize($_1944572090["PRICE_CODE"]) . "', '" . serialize($_1944572090["STORE"]) . "', NULL, NULL, NULL, NULL, NULL, NULL, " . $_1887043063 . ");");
            ...
            if ($_389040470 > 0) {
                ...
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_389040470 . ", 'UF_PHONE', '" . serialize($_1700836254) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_389040470 . ", 'UF_ADDRESS', '" . Loc::getMessage("sotbit.regions_ADDRESS", array("#HOME#" => rand(1, 50))) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_389040470 . ", 'UF_EMAIL', '" . serialize(["sales@" . $_1667311065 . $_1860535200]) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_389040470 . ", 'UF_ROBOTS', '');");


                if (Loader::includeModule('sale')) {
                    $_1822669924 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_LOCATION_' . $_69734081), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                    if ($_1822669924['ID'] > 0) {
                        $DB->Query("INSERT INTO `sotbit_regions_locations` VALUES (NULL," . $_389040470 . "," . $_1822669924["ID"] . ");");
                    }
                    if ($_69734081 == '') {
                        $_1822669924 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_LOCATION_MOSCOW'), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                    }
                    if ($_1822669924['ID'] > 0) {
                        $DB->Query("INSERT INTO `sotbit_regions_locations` VALUES (NULL," . $_389040470 . "," . $_1822669924["ID"] . ");");
                    }
    ...
    function SotbitRegionsInstallFavorites($_1260573971)
    {
        if (Loader::includeModule('sale')) {
            ...
            foreach ($_820219324 as $_915952566) {
                $_1822669924 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_FAVORITE_' . $_915952566), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                if ($_1822669924['CODE']) {
    ...
    function SotbitRegionsSetSettings($_1260573971)
    {
        global $DB;
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('SINGLE_DOMAIN','Y', '" . $_1260573971 . "');");
        if (Loader::includeModule('statistic')) {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','statistic', '" . $_1260573971 . "');");
        } elseif (function_exists('curl_version')) {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','ipgeobase', '" . $_1260573971 . "');");
        } else {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','geoip','" . $_1260573971 . "');");
        }
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('INSERT_SALE_LOCATION','N', '" . $_1260573971 . "');");
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('MULTIPLE_DELIMITER',', ', '" . $_1260573971 . "');");
        $_1396366420 = 5;
        if (Loader::includeModule('sale')) {
            $_1332076988 = TypeTable::getList(['select' => ['ID'], 'filter' => ['CODE' => 'CITY']])->fetch();
            if (!empty($_1332076988['ID'])) $_1396366420 = $_1332076988['ID'];
        }
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('LOCATION_TYPE', '" . $_1396366420 . "', '" . $_1260573971 . "');");
    }
    ...
    function installDB()
    {
        ...
        $_1057636470 = GeoIp\HandlerTable::add(['SORT' => 105, 'ACTIVE' => 'Y', 'CLASS_NAME' => '\\' . \Sotbit\Regions\SypexGeo\SypexGeoLocal::class, "CONFIG" => "",]);
//        $_1057636470 = GeoIp\HandlerTable::add(['SORT' => 105,  'ACTIVE' => 'Y', 'CLASS_NAME' => '\\Sotbit\Regions\SypexGeo\SypexGeoLocal::class, "CONFIG" => ""',]);
    }
    ...
    function UnInstallFiles()
    {
        ...
        \Bitrix\Main\IO\Directory::deleteDirectory(Application::getDocumentRoot() . '/local/templates/.default/components/sotbit/regions.choose');
        if (is_dir($_212967155 = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . self::MODULE_ID . '/install/themes/.default/')) {
            if ($_348137096 = opendir($_212967155)) {
                while (false !== $_1271387824 = readdir($_348137096)) {
    ...
    function UnInstallDB()
    {
        ...
        $_2082569957 = GeoIp\HandlerTable::query()->where('CLASS_NAME', '\\' . \Sotbit\Regions\SypexGeo\SypexGeoLocal::class)->fetchObject();
//        $_2082569957 = GeoIp\HandlerTable::query()->where('CLASS_NAME', '\\Sotbit\Regions\SypexGeo\SypexGeoLocal::class)->fetchObject();
...
//        EventManager::getInstance()->registerEventHandler('main', 'onMainGeoIpHandlersBuildList', 'sotbit.regions', '\\Sotbit\Regions\EventHandlers::class, "addGeoIpServes"',);
    ...
    {
        $_135703835 = new \CUserTypeEntity;
        $_1516249333 = $_135703835->GetList(array(), array('ENTITY_ID' => 'SOTBIT_REGIONS'));
    ...
    function SotbitRegionsGetAllFiles($_1602241508)
    {
    }

    function SotbitRegionsDeleteFiles($_671367095)
    {
    }
}