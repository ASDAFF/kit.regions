...
class SypexGeoUpdater {
    ...
    public static function download(string $lang="utf8", string $path2log='sypexGeoUpdate.log')
    {
        ...
        // Настройки
        $url = "https://sypexgeo.net/files/SxGeoCity_{$dowloadCharSet}.zip";  // Путь к скачиваемому файлу
        ...
        $dat_file = $types[$type];
        if (INFO) {
            file_put_contents($path2log, $dateString . ' => ' . "Скачиваем архив с сервера\n", FILE_APPEND);
        }
        ...
        if(!curl_exec($ch)) {
            file_put_contents($path2log, $dateString . ' => ' . "Ошибка при скачивании архива\n", FILE_APPEND);
            return;
        }
        ...
        if ($code == 304) {
            ...
            if (INFO) {
                file_put_contents($path2log, $dateString . ' => ' . "Архив не обновился с момента предыдущего скачивания\n", FILE_APPEND);
                return;
            }
        }

        if (INFO) {
            file_put_contents($path2log, $dateString . ' => ' . "Архив с сервера скачан\n", FILE_APPEND);
        }
        // Распаковываем архив
        $fp = fopen('zip://' . __DIR__ . '/SxGeoTmp.zip#' . $dat_file, 'rb');
        $fw = fopen($dat_file, 'wb');
        if (!$fp) {
            file_put_contents($path2log, $dateString . ' => ' . "Не получается открыть\n", FILE_APPEND);
            return;
        }
        if (INFO) {
            file_put_contents($path2log, $dateString . ' => ' . "Распаковываем архив\n", FILE_APPEND);
        };
        stream_copy_to_stream($fp, $fw);
        fclose($fp);
        fclose($fw);
        if(filesize($dat_file) == 0) {
            file_put_contents($path2log, $dateString . ' => ' . "Ошибка при распаковке архива\n", FILE_APPEND);
            return;
        }
        @unlink(__DIR__ . '/SxGeoTmp.zip');
        $res = rename(__DIR__ . '/' . $dat_file, $dat_file_dir . $dat_file);
        if (!$res) {
            file_put_contents($path2log, $dateString . ' => ' . "Ошибка при переименовании файла\n", FILE_APPEND);
            return;
        }
        file_put_contents($last_updated_file, gmdate('D, d M Y H:i:s') . ' GMT');
        if (INFO) {
            file_put_contents($path2log, $dateString . ' => ' . "Перемещен файл в {$dat_file_dir}{$dat_file}\n", FILE_APPEND);
        }
    }
    ...
    {
        \CAgent::RemoveModuleAgents('sotbit.regions');
    }
