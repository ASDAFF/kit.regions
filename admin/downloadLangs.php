    ...
    input.addEventListener('change', function(e) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (!types.includes(files[i].type)) {
                alert('<?=Loc::getMessage('kit.regions_ADMIN_ERROR_TYPE_FILES')?>');
                input.parentNode.querySelector('span').innerText = '<?=Loc::getMessage('kit.regions_ADMIN_ADD_FILE')?>';
            ...
            BX.ajax.runAction('kit:regions.AdminController.downloadLocationNames', {
                data: data,
            })
            .then(function(res) {
                input.parentNode.querySelector('span').innerText = '<?=Loc::getMessage('kit.regions_ADMIN_ADD_FILE')?>';
